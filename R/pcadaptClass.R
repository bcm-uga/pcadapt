#' pcadapt objects
#'
#' \code{create.pcadapt} computes the numerical quantities needed to compute the 
...
#' 
#' @return The returned value is a list containing all the numerical quantities 
#' needed to create an object of class \code{pcadapt}.
#' 
...
create.pcadapt = function(input, K, method, min.maf, ploidy, type){
  if (type == 0){
    ...
    aux <- cmpt_cov_cpp(filename = input, 
    ...
    unused_string <- "null" 
    aux <- cmpt_cov_cpp(filename = unused_string, 
