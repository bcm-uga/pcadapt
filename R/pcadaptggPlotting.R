...
#' Principal Components Analysis Scores Plot
#'
#' \code{"score.plotly"} plots the projection of the individuals onto the 
#' first two principal components.
#'
#' @param x an output from \code{pcadapt} containing the scores.
#' @param i an integer indicating onto which principal component the individuals
...
#' @export
#'
score.plotly = function(x, i = 1, j = 2, pop, gg.col, ind){
  
  if (attr(x, "K") == 1){
    warning("K = 1, option not available since two principal components have to 
            be computed at least.")
  } else {
