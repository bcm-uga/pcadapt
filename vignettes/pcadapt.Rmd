...
**NB**: to convert VCF files, you can use `type="vcf"` or `type="vcfR"`, the first option being substantially faster than the second option, while `type="vcfR"` works more consistantly than `type="vcf"`.
...
## Changelog `pcadapt 3.0.3` (10-06-2016)

- Option `type="vcfR"` has been added to `read.pcadapt` to overcome some conversion issues occuring with VCF files.
