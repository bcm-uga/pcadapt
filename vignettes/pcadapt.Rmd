...
The current version of `read.pcadapt` supports four formats: "vcf", "ped", "lfmm" and "pcadapt". For example, assume your genotype file is called "foo.lfmm" and is located in the directory "path_to_directory", use the following command lines:
...
A new argument has been added to `read.pcadapt`, in order to specify whether the genotype matrix has to be read from a file or from the local environment. By default, `local.env=TRUE`, meaning that `read.pcadapt` will read the genotype matrix from the file accordingly. If the genotype matrix is already loaded in the global environment, in the `lfmm` format or in the `pcadapt` format, running `read.pcadapt` will write an output file and returns the name of the output file. For example, if your genotype matrix is stored under the variable name `geno.matrix` and its format is `lfmm`, you should use the following command line:

```
filename <- read.pcadapt(geno.matrix,type="lfmm",local.env=TRUE)
```

## A.3. How to handle missing values
...
## G.1. Computing other test statistics
