...
For pooled samples, the package also uses the `read.pcadapt` function. 
...
With Pool-Seq data, the package computes a Mahalanobis distance to search for **outlier vector of allele frequencies**. Computing PCs, as for genotype data, is not needed anymore.  Denoting by $f^j = (f_1^j, \dots, f_K^j)$ the vector of allele frequencies in the `K` populations for the $j$-th SNP, the squared Mahalanobis distance for pool-seq data is defined as

$$D_j^2 = (f^j - \bar{f})\Sigma^{-1}(f^j - \bar{f})$$

where $\bar{z}$ and $\Sigma$ are robust estimates of the mean and of the covariance matrix of allele frequencies. Dividing again the squared Mahalanobis distance by a genomic inflation factor provides a test statistic that should distributed as a chi-square distribution with $K$ degrees of freedom. Computation of the Mahalanobis distance is performed as follows

```{r,  eval = TRUE}
res<-pcadapt(filename)
summary(res)
```
- `chi2.stat` is a vector of size `L` containing the rescaled statistics that follow a chi-squared distribution with `K` degrees of freedom.

- `gif` is a numerical value corresponding to the genomic inflation factor estimated from Mahalanobis distances.

- `pvalues` is a vector containing `L` p-values.

A Manhattan plot and a list of outliers can then be obtained 
```{r,  eval = TRUE}
...
### H.2. Run `pcadapt` on matrices load in memory
 
`pcadapt` can be called on matrices load in memory instead of using a file.  When calling the function `read.pcadapt`, users should specify if individuals are in columns, and SNPs in lines (type = "pcadapt") or if individuals are in lines, and SNPs in columns (type = `lfmm`).
```{r, eval=FALSE}
path_to_file <- system.file("extdata", "SSMPG2017.rds", package = "pcadapt")
genotypes<-readRDS(path_to_file)
print(dim(genotypes))
...
|           |                  | - Computation of PCA is based on the packages `Rspectra`.|
...
|           |                  | - For Pooled-seq data, use Mahalanobis distances directly on the vector of frequencies, no more simulations of individuals genotypes, no more PCA for Pooled-Seq data|
