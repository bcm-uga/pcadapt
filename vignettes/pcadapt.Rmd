...
<center> <h1> How to use pcadapt to detect local adaption with individual genotype data. </h1> </center>
...
A new argument has been added to `read.pcadapt`, in order to specify whether the genotype matrix has to be read from a file or from the local environment. By default, `local.env=FALSE`, meaning that `read.pcadapt` reads genotype data stored in text files by default. If the genotype matrix is already loaded in the global environment, in the `lfmm` format or in the `pcadapt` format, running `read.pcadapt` will write an output file and will return the name of the output file. For example, if your genotype matrix is stored under the variable name `geno.matrix` and is of type `lfmm`, you should use the following command line:
...
### F.1. Computing other test statistics
