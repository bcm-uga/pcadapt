...
Here we present version 4 of pcadapt which substantially improves computational efficiency. 
...
Previous versions of package pcadapt used format `pcadapt', which is a text file of characters separated by spaces where each line stores all individuals' genotypes for one variant (0, 1, 2, and 9 for missing values).
...
Function \texttt{pcadapt} v3.0.4 takes 2111 seconds (35 minutes) to complete for this data. The bulk of this time is spent computing the GRM ($O(N^2 P)$, where $N$ is the number of individuals and $P$ the number of variants).
