...
\end{equation}
where $\widetilde{G}$ is the scaled genotype matrix and $\delta_{ik}$ is an indicator if the genotype of individual $i$ and variant $k$ is available ($\delta_{ik} = 1$) or missing ($\delta_{ik} = 0$).
